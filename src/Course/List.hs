...
flatten ::
  ...
  -> List a
-- flatten Nil = Nil
-- flatten ( (x :. xs) :. lists) = x :. flatten (xs :. lists)
-- flatten ( Nil :. lists) = flatten lists
...
reverse ::
  ...
  -> List a
-- reverse Nil = Nil
-- reverse list = rev' Nil list
--   where
--     rev' out Nil = out
--     rev' out (x :. xs) = rev' (x :. out) xs
...
notReverse ::
  List a
  -> List a
notReverse = reverse -- because of the property notReverse x ++ notReverse y == notReverse (y ++ x)
